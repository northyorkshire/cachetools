...
class TLRUCacheTest(unittest.TestCase, CacheTestMixin):
    ...
    def test_ttu_lru(self):
        cache = TLRUCache(maxsize=2, ttu=lambda _: 0, timer=Timer())
    ...
    def test_ttu_expire(self):
        cache = TLRUCache(maxsize=3, ttu=lambda _: 2, timer=Timer())
    ...
    def test_ttu_atomic(self):
        cache = TLRUCache(maxsize=1, ttu=lambda _: 1, timer=Timer(auto=True))
        cache[1] = 1
        self.assertEqual(1, cache[1])
        cache[1] = 1
    ...
    def test_ttu_tuple_key(self):
        cache = TLRUCache(maxsize=1, ttu=lambda _: 0, timer=Timer())
